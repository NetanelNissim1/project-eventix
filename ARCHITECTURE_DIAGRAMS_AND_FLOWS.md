# Project Eventix: System Architecture & Flow Diagrams

This document provides a comprehensive visual and technical specification of **Project Eventix**, illustrating the system topology, component interactions, distributed patterns, data pipelines, and error handling flows.
...
## 2. Network & Port Allocation Map

```mermaid
flowchart LR
    subgraph Host_Machine ["Host Machine (Developer Workstation)"]
        Browser["Web Browser\nClient (Port 5173)"]
...
## 3. End-to-End Choreography Saga Flow (Happy Path)

This diagram details the successful lifecycle of an order from initial submission to final confirmation.
...
sequenceDiagram
    ...
    Customer->>GW: POST /api/v1/orders (Order Items, Idempotency Key)
    ...
    rect rgb(245, 255, 245)
        Note over InvSvc,Redis: Concurrency Defense
...
## 4. Saga Compensation Flows (Failure Scenarios)
...
## 5. Transactional Outbox Pattern & Debezium CDC Pipeline
...
## 6. Distributed Locking Concurrency Architecture (Redisson)

To prevent overselling and race conditions when multiple users purchase the same high-demand product simultaneously:
...
## 7. Edge Rate Limiting Pipeline (Redis Token Bucket)
...
## 8. Security & Data Protection Pipeline (GDPR / PCI-DSS)
...
## 9. Scheduled Aggregation & Daily Digest Flow
...
## 10. Database-Per-Service Topology
